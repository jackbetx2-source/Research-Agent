...
class ResearchWebHandler(BaseHTTPRequestHandler):
    ...
    def _handle_slr(self) -> None:
        try:
            ...
            if not isinstance(user_papers, list):
                user_papers = []
            user_papers, llm_context_documents = self._augment_references_with_llm(
                user_papers,
                topic,
                purpose="systematic literature review",
            )
            if llm_context_documents:
                topic = self._build_uploaded_context_topic(topic, llm_context_documents)
            job_id = uuid.uuid4().hex
            with JOBS_LOCK:
    ...
    def _handle_slr_upload(self) -> None:
        try:
            ...
            references, llm_context_documents = self._augment_references_with_llm(
                references,
                topic,
                purpose="systematic literature review upload",
            )
            uploaded_references = [
    ...
    def _run_job(
        ...
        try:
            ...
            if references != result.references:
                result = replace(result, references=references)
            final_report = ResearchWorkflow.append_apa_references(
                result.final_report,
                result.references,
            )
            if final_report != result.final_report:
                result = replace(result, final_report=final_report)
            self._save_result(topic, result)
            with JOBS_LOCK:
                JOBS[job_id] = {
                    "status": "done",
    ...
    def _run_slr_job(
        ...
        try:
            ...
            self._save_slr_result(topic, result.report_markdown)
            with JOBS_LOCK:
                JOBS[job_id] = {
                    "status": "done",
                    "kind": "slr",
    ...
    async def _answer_project_question(self, message: str, history: list) -> str:
        ...
        system_prompt = (
            "You are the built-in assistant for a Research Agent web application. "
        ...
        user_prompt = (
            "Project facts:\n"
            "- The app is a Python-backed web application with a static browser UI, suitable for development or server deployment.\n"
            ...
            "- Results are shown in the browser and can be exported from the web UI.\n"
...
""".strip()
    ...
    def _build_high_information_package(text: str, *, max_chars: int = PDF_REFERENCE_EXCERPT_CHARS) -> str:
        return SLRWorkflow._high_information_excerpt(text, max_chars=max_chars)
    ...
    def _markdown_to_pdf_bytes(title: str, markdown: str) -> bytes:
        ...
        def append_table(table_lines: list[str]) -> None:
            ...
            if len(rows) < 2:
                return
            data = [
                [Paragraph(ResearchWebHandler._pdf_inline_text(cell), normal) for cell in row]
