...
class ResearchWebHandler(BaseHTTPRequestHandler):
    ...
    def _handle_slr_upload(self) -> None:
        try:
            ...
            references.extend(self._uploaded_file_to_reference(filename, content) for filename, content in files)
            ...
            thread.start()
            self._send_json({"job_id": job_id, "status": "queued", "references": references}, HTTPStatus.ACCEPTED)
        except BrokenPipeError:
    ...
    def _handle_literature_pdf_analysis(self) -> None:
        try:
            ...
            if not references and not user_context:
                self._send_json(
                    {"error": "Please provide references, uploaded files, or text context."},
                    HTTPStatus.BAD_REQUEST,
                )
